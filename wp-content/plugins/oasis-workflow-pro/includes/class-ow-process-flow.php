...
class OW_Process_Flow {
    ...
    */
	...
	public function validate_submit_to_workflow() {
		...
		$is_bypass_warning = intval( $_POST['by_pass_warning'] );
		...
		if ( $validation_result['error_type'] == "warning" ) {
			...
			$continue_to_submit_label     = ! empty( $workflow_terminology_options['continueToSubmitText'] )
				? $workflow_terminology_options['continueToSubmitText'] : __( 'Continue to Submit', 'oasisworkflow' );
	...
	public function submit_post_to_step() {
		...
		if ( $validation_result['error_type'] == "warning" ) {
			...
			$continue_to_signoff_label = ! empty( $workflow_terminology_options['continueToSignoffText'] )
				...
				: __( 'Continue to Sign off', 'oasisworkflow' );

			$continue_to_signoff_button = '<p class="owf-wrapper"><input type="button" value="' .
			                              $continue_to_signoff_label .
			                              '" class="bypassWarning button button-primary" /></p>';
		}
	...
	private function validate_workflow_sign_off( $post_id, $sign_off_workflow_params ) {
		...
		if ( ! empty( $team_id ) ) {
			if ( ! $this->has_users_in_team( $sign_off_workflow_params["post_id"],
				...
				$team_error_message
					= __( 'No users found for the assigned Team and Workflow role. Please contact the administrator.',
					'oasisworkflow' );
    ...
    */
	...
	private function submit_post_to_step_internal( $post_id, $workflow_signoff_data ) {
		...
		if ( $history_details->assign_actor_id ==
			...
			if ( ! empty( $workflow_signoff_data['due_date'] ) ) {
				$review_data["due_date"] = OW_Utility::instance()
				                                     ->format_date_for_db_wp_default( $workflow_signoff_data['due_date'] );
			}
			...
			if ( ! empty( $workflow_signoff_data['due_date'] ) ) {
				$data["due_date"] = OW_Utility::instance()
				                              ->format_date_for_db_wp_default( $workflow_signoff_data['due_date'] );
			}
	...
	public function api_submit_to_step( $data ) {
		...
		if ( ! current_user_can( 'ow_sign_off_step' ) ) {
			return new WP_Error( 'owf_rest_submit_to_step', __( 'You are not allowed to signoff.', 'oasisworkflow' ),
				array( 'status' => '403' ) );
		}
	...
	public function check_for_claim( $action_history_id ) {
		...
		$rows               = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . OW_Utility::instance()
		                                                                                       ->get_action_history_table_name() .
	...
	public function api_check_for_claim( $data ) {
		...
		if ( ! current_user_can( 'ow_sign_off_step' ) ) {
			return new WP_Error( 'owf_rest_check_for_claim', __( 'You are not allowed to claim.', 'oasisworkflow' ),
				array( 'status' => '403' ) );
		}
	...
	public function api_claim_process( $claim_data ) {
		...
		if ( ! current_user_can( 'ow_sign_off_step' ) ) {
			return new WP_Error( 'owf_rest_claim_process',
				__( 'You are not allowed to claim the task.', 'oasisworkflow' ), array( 'status' => '403' ) );
		}
	...
	public function claim_process( $claim_data = null ) {
		...
		if ( $this->check_for_claim( $action_history_id ) ) { // First check if "claim" is applicable or not
			...
			foreach ( $action_histories as $action )
			{ // for all the history ids, only one will be "claimed", rest need to be "unclaimed" OR claim_cancelled.
				if ( $post_title == "" ) {
			...
			$validation_result[] = __( "Sorry, You can't claim the task. It is already claimed by another user.",
				'oasisworkflow' );
			$error_message       = $this->construct_claim_error_message( $validation_result );
			if ( $is_api == true ) {
				return array( 'isError' => 'true', 'errorMessage' => $validation_result );
		...
		if ( $is_api == true ) {
			...
			$link       = admin_url() . "post.php?post=" . $post_id . "&action=edit&oasiswf=" . $new_history_id .
			              "&user=" . $selected_user;
    ...
    */
	...
	public function api_reassign_process( $data ) {
		...
		if ( ! current_user_can( 'ow_reassign_task' ) ) {
			return new WP_Error( 'owf_rest_reassign_task',
				__( 'You are not allowed to reassign task.', 'oasisworkflow' ), array( 'status' => '403' ) );
		}
	...
	public function reassign_process( $reassign_data = null ) {
		...
		if ( empty( $reassign_data ) ) {
			...
			/* sanitize incoming data */
			$current_user      = isset( $_POST["task_user"] ) && $_POST["task_user"] !== ""
			...
			$reassign_comments = ( sanitize_text_field( $_POST['reassign_comments'] ) != "" )
				? sanitize_text_field( $_POST['reassign_comments'] ) : "";

		} else {
			/* sanitize incoming data */
			$current_user      = ( sanitize_text_field( $reassign_data["task_user"] ) != "" )
				? intval( sanitize_text_field( $reassign_data["task_user"] ) ) : get_current_user_id();
			...
			$reassign_comments = ( sanitize_text_field( $reassign_data['comments'] ) != "" )
    ...
    */
	...
	public function api_workflow_complete( $data ) {
		...
		if ( ! current_user_can( 'ow_sign_off_step' ) ) {
			return new WP_Error( 'owf_rest_workflow_complete',
				__( 'You are not allowed to publish post.', 'oasisworkflow' ), array( 'status' => '403' ) );
		}
	...
	public function api_workflow_cancel( $data ) {
		...
		if ( ! current_user_can( 'ow_abort_workflow' ) ) {
			return new WP_Error( 'owf_rest_workflow_cancel',
				__( 'You are not allowed to end the workflow process.', 'oasisworkflow' ), array( 'status' => '403' ) );
		}
	...
	public function workflow_cancel( $api_data = null ) {
		...
		if ( $new_action_history_id ) {
			...
			$msg
				= sprintf( __( '<div>Hello %1$s,</div><p>The post <a href="%2$s" title="%3$s">%3$s</a> has been cancelled from the workflow.</p>',
				'oasisworkflow' ),
			...
			if ( ! empty ( $user_comments ) ) {
				$msg .= "<p><strong>" . __( 'Additionally,', "oasisworkflow" ) . "</strong> {$full_name} " .
				        __( 'added the following comments', "oasisworkflow" ) . ":</p>";
			...
			if ( $is_api ) {
				$response = array(
					"success_response" => __( 'The workflow was successfully aborted from the last step.',
						'oasisworkflow' )
				);
	...
	public function api_workflow_abort( $data ) {
		...
		if ( ! current_user_can( 'ow_abort_workflow' ) ) {
			return new WP_Error( 'owf_rest_abort_workflow',
				__( 'You are not allowed to abort the workflow.', 'oasisworkflow' ), array( 'status' => '403' ) );
		}
	...
	public function api_delete_post( $data ) {
		...
		if ( ! current_user_can( 'ow_make_revision' ) || ! current_user_can( 'ow_make_revision_others' ) ) {
			return new WP_Error( 'owf_rest_delete_revision_post',
				__( 'You are not allowed delete the post', 'oasisworkflow' ), array( 'status' => '403' ) );
		}
		$post_id = intval( $data['post_id'] );
		if ( $post_id ) {
			$status = wp_trash_post( $post_id ) ? 'success' : 'error';
    ...
    */
	...
	public function when_post_trash_delete( $post_id ) {
		...
		if ( $histories ) {
			foreach ( $histories as $history ) {
				$wpdb->get_results( $wpdb->prepare( "DELETE FROM " . OW_Utility::instance()->get_action_table_name() .
				                                    " WHERE action_history_id = %d", $history->ID ) );
				$wpdb->get_results( $wpdb->prepare( "DELETE FROM " . OW_Utility::instance()->get_emails_table_name() .
			...
			$wpdb->get_results( $wpdb->prepare( "DELETE FROM " .
			                                    OW_Utility::instance()->get_action_history_table_name() .
	...
	public function pre_validate_claim( $validation_result, $action_history_id, $post_id, $user_id ) {
		...
		if ( $assigned_task_count >= 1 ) {
			$error_messages[]
				               = __( 'You cannot claim additional tasks, since you already have more than 2 assignments.',
				'oasisworkflow' );
	...
	public function show_only_accessible_posts( $query ) {
		...
		if ( is_admin() ) {
             ...
             */
				...
				$sql = "SELECT A.post_id as available_post_id FROM " .
				       OW_Utility::instance()->get_action_history_table_name() . " A
               LEFT OUTER JOIN " . OW_Utility::instance()->get_action_table_name() . " B ON A.ID = B.action_history_id
                  AND B.review_status = 'assignment'
    ...
    */
	...
	public function get_submitted_article_count( $post_type = 'all', $team_filter ) {
		...
		if ( $post_type === "all" ) {
			...
			$sql             = "SELECT count(posts.ID) as post_id FROM " . $wpdb->posts .
			                   " as posts $join WHERE post_type = %s AND ID IN (" . implode( $assign_post_ids, "," ) .
			                   ") $where ";
	...
	public function api_submit_to_workflow( $data ) {
		...
		if ( ! current_user_can( 'ow_submit_to_workflow' ) ) {
			return new WP_Error( 'owf_rest_submit_to_workflow',
				__( 'You are not allowed to submit to workflow.', 'oasisworkflow' ), array( 'status' => '403' ) );
		}
		...
		$user_provided_publish_date = isset( $publish_date ) ? date( OASISWF_DATE_TIME_FORMAT,
    ...
    */
	...
	public function api_get_reassign_assignees( $data ) {
		...
		if ( ! current_user_can( 'ow_reassign_task' ) ) {
			return new WP_Error( 'owf_rest_reassign', __( 'You are not allowed to reassign task.', 'oasisworkflow' ),
				array( 'status' => '403' ) );
		}
		...
		if ( get_option( "oasiswf_activate_workflow" ) == "active" &&
			...
			if ( $chkResult == "inbox" ) {
				...
				$inbox_service->enqueue_and_localize_script();
			           ...
			           $chkResult == "submit" &&
			           is_admin() &&
			           preg_match_all( '/page=oasiswf(.*)|post-new\.(.*)|post\.(.*)/', $_SERVER['REQUEST_URI'],
				           $matches ) ) {
			...
			if ( isset( $_GET['post'] ) && sanitize_text_field( $_GET["post"] ) && isset( $_GET['action'] ) &&
				...
				if ( get_post_meta( $post_id, '_oasis_original', true ) ) {
					$aborted = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM " .
					                                           OW_Utility::instance()->get_action_history_table_name() .
			...
			echo "<input type='hidden' name='owf_claim_process_ajax_nonce' id='owf_claim_process_ajax_nonce' value='" .
			     wp_create_nonce( 'owf_claim_process_ajax_nonce' ) . "'/>";
    ...
    */
	...
	public function enqueue_and_localize_submit_step_script() {
		...
		wp_localize_script( 'owf_submit_step', 'owf_submit_step_vars', array(
			...
			'lastStepFailureMessage'  => __( 'There are no further steps defined in the workflow.</br> Do you want to cancel the post/page from the workflow?',
				'oasisworkflow' ),
			'lastStepSuccessMessage'  => __( 'This is the last step in the workflow. Are you ready to complete the workflow?',
				'oasisworkflow' ),
			...
			'multipleUsers'           => __( 'You can select multiple users only for review step. Selected step is',
				'oasisworkflow' ),
			...
			'noRoleUsersInTeam'       => __( 'No users found for the assigned Team and Workflow assignee(s). Please contact the administrator.',
				'oasisworkflow' ),
			...
			'elementorSignoffText'    => __( 'The task was successfully signed off.', 'oasisworkflow' ),
			'elementorPublishText'    => __( 'The post was successfully published.', 'oasisworkflow' ),
			'elementorExitButtonText' => __( 'Exit to Dashboard', 'oasisworkflow' )
		) );
	}
    ...
    */
	...
	public function enqueue_and_localize_submit_workflow_script() {
		...
		$submit_to_workflow_label     = ! empty( $workflow_terminology_options['submitToWorkflowText'] )
			? $workflow_terminology_options['submitToWorkflowText'] : __( 'Submit to Workflow', 'oasisworkflow' );
		wp_localize_script( 'owf_submit_workflow', 'owf_submit_workflow_vars', array(
			...
			'noRoleUsersInTeam'           => __( 'No users found for the given Team and Workflow assignee(s). Please check the team.',
				'oasisworkflow' ),
			'compareOriginal'             => __( 'Compare With Original', 'oasisworkflow' ),
			'dateFormat'                  => OW_Utility::instance()
			                                           ->owf_date_format_to_jquery_ui_format( get_option( 'date_format' ) ),
			...
			'isCommentsMandotory'         => get_option( "oasiswf_comments_setting" ),
			'emptyComments'               => __( 'Please add comments.', 'oasisworkflow' ),
			'elementorWorkflowSubmitText' => __( 'The post was successfully submitted to the workflow.',
				'oasisworkflow' ),
			'elementorExitButtonText'     => __( 'Exit to Dashboard', 'oasisworkflow' )
		) );
		//}
    ...
    */
	...
	public function enqueue_and_localize_make_revision_script() {
		...
		$make_revision_label          = ! empty( $workflow_terminology_options['makeRevisionText'] )
			? $workflow_terminology_options['makeRevisionText'] : __( 'Make Revision', 'oasisworkflow' );
    ...
    */
	...
	public function api_check_is_role_applicable( $criteria ) {
		...
		if ( ! current_user_can( 'ow_submit_to_workflow' ) && ! current_user_can( 'ow_sign_off_step' ) ) {
			return new WP_Error( 'owf_rest_check_role_capability',
				__( 'You are not allowed to get workflow step details.', 'oasisworkflow' ),
				array( 'status' => '403' ) );
		}
	...
	public function elementor_submit_to_workflow() {
		...
		$priority = sanitize_text_field( $_POST["priority_select"] );
		if ( empty( $priority ) ) {
			$priority = '2normal';
		}

		$selected_actor_val = sanitize_text_field( $_POST["actor_ids"] );
		OW_Utility::instance()->logger( "User Provided Actors:" . $selected_actor_val );

		$team_addon = sanitize_text_field( $_POST["is_team_available"] );
		$team_id    = null;
		$is_team    = false;
		// if teams add-on is active
		if ( $team_addon === "true" ) {
		...
		// sanitize_text_field remove line-breaks so do not sanitize it.
		$comments = $this->sanitize_comments( nl2br( $_POST["comment"] ) );

		$publish_date               = sanitize_text_field( $_POST["publish_datetime"] );
