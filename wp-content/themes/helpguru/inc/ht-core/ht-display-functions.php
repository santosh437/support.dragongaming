...
if(!function_exists('ht_breadcrumb_display')){
	...
	function ht_breadcrumb_display( $sep = '<span class="sep">&gt;</span>' ) {
	    ...
	    if (!is_front_page() && $ht_show_breadcrumbs) {  
	    	...
	    	$home_link = '<a href="' . home_url() . '" class="ht-breadcrumbs-home">' . __('DragonGaming Support', 'helpguru') . '</a> ' . $sep;
...
//		            $cat = get_category_parents(get_query_var('cat'), true, $sep);
                    $cat = str_replace('/'.get_category(get_query_var('cat'))->taxonomy, '',get_category_parents(get_query_var('cat'), true, $sep));
				...
				if( !empty($terms) ){
					foreach($terms as $term) {
						...
						if ($term->parent != 0) { 
							...
							echo str_replace('/'.$term->taxonomy, '', $parents);
						} else {
//							echo '<a href="' . esc_attr(get_term_link($term, 'category')) . '" title="' . sprintf( __( "View all posts in %s", 'helpguru' ), $term->name ) . '" ' . '>' . $term->name.'</a> ';
//							echo '<a href="' . $term->slug . '" title="' . sprintf( __( "View all posts in %s", 'helpguru' ), $term->name ) . '" ' . '>' . $term->name.'</a> ';
							echo '<a href="'.path_join(home_url(), $term->slug).'" title="' . sprintf( __( "View all posts in %s", 'helpguru' ), $term->name ) . '" ' . '>' . $term->name.'</a> ';
							echo wp_filter_post_kses($sep);
...
if ($ht_sidebar_position == '') {
	echo esc_attr('sidebar-right');
} else {
	echo esc_attr($ht_sidebar_position);
}
