...
class OW_Email {
		...
		if ( $email_settings['reminder_emails'] == "yes" ) {
			...
			if ( $email_settings['digest_emails'] == "yes" ) {
				...
				$rows = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " .
				                                            OW_Utility::instance()->get_emails_table_name() .
	...
	public function get_step_mail_content( $action_id, $step_id, $to_user_id, $post_id ) {
		...
		if ( $step && $post ) {
			...
			$messages->assign_subject = ( ! empty( $subject_line ) ) ? $messages->assign_subject
				: $blog_name . __( "You have an assignment", "oasisworkflow" );
			$messages->assign_content = ( ! empty( $content_line ) ) ? $messages->assign_content
				: __( "You have an assignment related to post - ", "oasisworkflow" ) . $post_link;
			...
			foreach ( $messages as $k => $v ) {
				...
				if ( $k === "assign_subject" || $k ===
				                                "reminder_subject" ) { // since its a email subject, we don't need to have a link to the post
					$v = str_replace( OW_Place_Holders::POST_TITLE,
	...
	public function get_user_sign_off_comments( $action_history_id, $comment_object, $post_id ) {
		...
		if ( $comment_object->comment != "" ) {
			$comments_str .= "<p><strong>" . __( 'Additionally,', "oasisworkflow" ) . "</strong> {$full_name} " .
	...
	public function delete_step_email( $action_history_id, $user_id = null ) {
		...
		if ( $user_id ) {
			$sql = "DELETE FROM " . OW_Utility::instance()->get_emails_table_name() .
	...
	public function send_digest_email( $email_type ) {
		...
		$rows = $wpdb->get_results( $wpdb->prepare( "SELECT to_user, subject, message FROM " .
		                                            OW_Utility::instance()->get_emails_table_name() .
