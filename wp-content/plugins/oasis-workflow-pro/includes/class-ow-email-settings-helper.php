...
class OW_Email_Settings_Helper {
	...
	public function get_email_type_dropdown( $sel_type = 'post_publish' ) {
		...
		foreach ( $email_type as $key => $val ) {
			$option .= '<option value="' . $key . '" ' . selected( $sel_type, $key, false ) . '>' .
	...
	public function get_unauthorized_update_content() {
		...
		$default_email_body .= '{current_user}, ' .
		                       __( 'who is not part of the assignee list has updated the article ', 'oasisworkflow' ) .
		                       '{post_title}' . '';
	...
	public function get_task_claimed_content() {
		$default_email_body = __( 'Hello ', 'oasisworkflow' ) . '{first_name}' . ",\n\n";
		$default_email_body .= __( 'Another user has claimed the task for the article ', 'oasisworkflow' ) .
		                       '{post_title}' . '.';
	...
	public function get_post_submit_content() {
		$default_email_body = __( 'Hello ', 'oasisworkflow' ) . '{first_name}' . ",\n\n";
		$default_email_body .= __( 'Your article ', 'oasisworkflow' ) . '{post_title}' . '';
		$default_email_body .= __( ' has been successfully submitted to the workflow on ', 'oasisworkflow' ) .
		                       '{blog_name}' . ".\n\n";
		$default_email_body .= __( 'Thanks.', 'oasisworkflow' );
	...
	public function get_workflow_abort_content() {
		...
		$default_email_body .= __( ' has been aborted from the workflow on ', 'oasisworkflow' ) . '{blog_name}' .
		                       ".\n\n";
		$default_email_body .= __( 'If you have further questions regarding your article, please contact the administrator. ',
				'oasisworkflow' ) . "\n\n";
		$default_email_body .= __( 'Thanks.', 'oasisworkflow' );
            ...
            <ul>
                <li>
                   <span class="description">
                      <?php echo __( "Available template placeholders for email subject and content (Applicable to email recipients in the 'To' list only):",
	                      "oasisworkflow" ); ?>
                   </span>
                </li>
				<?php
	...
	private function get_workflow_submitter( $post_id ) {
		...
		$user_ids = array();
		$row      = $wpdb->get_row( $wpdb->prepare( "SELECT assign_actor_id FROM " .
