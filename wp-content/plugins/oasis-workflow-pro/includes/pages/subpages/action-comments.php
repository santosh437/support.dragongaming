...
<div id="ow-editorial-readonly-comment-popup">
    <div id="ow-comment-popup" class="ow-modal-dialog ow-top_15">
        ...
        <div class="ow-modal-header">
            <h3 class="ow-modal-title" id="poststuff"><?php echo __( 'Editorial Comments On: ' ) .
							...
							if ( $object ) {
								foreach ( $object as $comment ) {
									...
									if ( ! empty ( $comment->comment_timestamp ) ) {
										$timestamp = OW_Utility::instance()
										                       ->format_date_for_display( $comment->comment_timestamp,
											                       '-', 'datetime' );
									}
									?>
                                    <ul id="readonly-comments">
                                        ...
                                        <li>
                                            <div class="ow-signed-off">
                                                <p class="ow-sign-off-text"><?php _e( 'Signed off on',
														'oasisworkflow' ); ?> <span
                                                        class="timestamp"><?php echo $timestamp; ?></span></p>
                                                <p class="ow-signed-off-comment"><?php echo $comment->comment !== ''
														? $comment->comment
														: _e( 'No Comments', 'oasisworkflow' ); ?></p>
                                            </div>
        ...
        <div class="ow-modal-footer">
            <a href="#" onclick="ow_modal_close(event);" class="modal-close"><?php _e( 'Close',
					'oasisworkflow' ); ?></a>
        </div>
    </div>
