...
class OW_Workflow_Service {
	...
	public function create_new_workflow() {
		...
		$result = $wpdb->get_row( $wpdb->prepare( "SELECT count(*) count FROM " .
		                                          OW_Utility::instance()->get_workflows_table_name() .
	...
	public function delete_workflow() {
		...
		$result = $wpdb->get_results( $wpdb->prepare( "DELETE FROM " .
		                                              OW_Utility::instance()->get_workflows_table_name() .
		                                              " WHERE ID = %d", $wf_id ) );
	...
	public function api_get_step_action_details( $data ) {
		...
		if ( ! current_user_can( 'ow_sign_off_step' ) ) {
			return new WP_Error( 'owf_rest_step_action_details',
				__( 'You are not allowed to get step process details.', 'oasisworkflow' ), array( 'status' => '403' ) );
		}
		...
		$signoff_action_success = isset( $step_info->signoff_success_action ) &&
		                          ( ! empty( $step_info->signoff_success_action ) ) ? $step_info->signoff_success_action
			: ( $process == "review" ? __( "Approved", "oasisworkflow" ) : __( "Complete", "oasisworkflow" ) );
		...
		if ( $show_failure_option ) {
			$signoff_action_failure = isset( $step_info->signoff_failure_action ) &&
				...
				: ( $process == "review" ? __( "Reject", "oasisworkflow" )
					: __( "Unable to Complete", "oasisworkflow" ) );
		}
	...
	public function upsert_workflow_step( OW_Workflow_Step $workflow_step ) {
		...
		if ( $result ) { // we are basically updating an existing step
			...
			$insert_row = $wpdb->get_row( "SELECT max(ID) as maxid FROM " .
			                              OW_Utility::instance()->get_workflow_steps_table_name() );
	...
	public function save_workflow( $button_clicked = "workflow_save_and_close" ) {
		...
		if ( isset( $_POST["start-date"] ) && ! empty( $_POST["start-date"] ) ) {
			$start_date = OW_Utility::instance()
			                        ->format_date_for_db_wp_default( sanitize_text_field( $_POST["start-date"] ) );
		}
		if ( isset( $_POST["end-date"] ) && ! empty( $_POST["end-date"] ) ) {
			$end_date = OW_Utility::instance()
			                      ->format_date_for_db_wp_default( sanitize_text_field( $_POST["end-date"] ) );
		}
		...
		if ( $parent_id > 0 ) {
			...
			$wpdb->query( $wpdb->prepare( "UPDATE $workflow_table SET end_date = '%s' WHERE ID = '%d'", $end_date,
				$parent_id ) );
		}
	...
	public function api_get_valid_workflows( $criteria ) {
		...
		if ( ! current_user_can( 'ow_submit_to_workflow' ) && ! current_user_can( 'ow_sign_off_step' ) ) {
			...
			return new WP_Error( 'owf_rest_validate_workflow',
				__( 'You are not allowed to fetch workflow settings.', 'oasisworkflow' ), array( 'status' => '403' ) );
		}
		...
		if ( $workflow ) {
			...
			if ( $wf_info ) {
				...
				foreach ( $wf_info->steps as $k => $v ) {
					...
					$result = $wpdb->get_results( $wpdb->prepare( "DELETE FROM " . OW_Utility::instance()
					                                                                         ->get_workflow_steps_table_name() .
					                                              " WHERE workflow_id = %d and ID = %d", $wf_id,
						$v->fc_dbid ) );
				}
			}
		}
	...
	private function get_next_version_number( $workflow_id ) {
		...
		$row             = $wpdb->get_row( $wpdb->prepare( "SELECT max(version) as current_max_version FROM " .
		                                                   OW_Utility::instance()->get_workflows_table_name() .
		                                                   " WHERE parent_id = %s OR ID = %s", $workflow_id,
			$workflow_id ) );
	...
	private function save_step_as_new( $new_wf_id, $current_step_id ) {
		...
		$result = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $workflow_step_table WHERE ID = %d",
			$current_step_id ) );
		if ( $result ) {
