...
class OW_Workflow_Validator {
	...
	public function validate_workflow_ajax() {
		...
		$auto_submit_site_teams_array = ( isset( $_POST["auto_submit_team"] ) && $_POST["auto_submit_team"] )
			? $_POST["auto_submit_team"] : '';
...
// 		$error_messages .= $this->validate_teams_for_auto_submit( $args['auto_submit_site_teams_array'] );
	...
	private function validate_steps( $wf_info ) {
		...
		if ( $wf_info->steps ) {
			foreach ( $wf_info->steps as $step ) {
				if ( $step->fc_dbid ==
					...
					$error_messages .= __( "\" step information. Right click on the step to edit step information.",
						"oasisworkflow" );
					$error_messages .= "<br>"; // add new line for new messages to append on new line
				} // end if
				$step_count ++;
		...
		if ( $steps != "nodefine" && count( $steps["first"] ) == 0 && count( $steps["last"] ) == 0 ) {
			$error_messages .= __( "The workflow doesn't have a valid exit path.	Items in this workflow will never exit the workflow. Please provide a valid exit path.",
				"oasisworkflow" );
			$error_messages .= "<br>"; // add new line for new messages to append on new line
		}

		if ( count( $wf_info->first_step ) > 1 ) {
			$error_messages .= __( 'Multiple steps marked as first step. Workflow can have only one starting point.',
				"oasisworkflow" );
			$error_messages .= "<br>"; // add new line for new messages to append on new line
		}
