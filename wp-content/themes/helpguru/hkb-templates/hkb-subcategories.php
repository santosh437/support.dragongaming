    ...
    <ul class="hkb-subcats">
        <?php foreach ($subcategories as $term): ?>
            <li class="hkb-subcats__cat">
                ...
                <a class="hkb-subcats__cat-title" href="<?php echo esc_attr(get_term_link($term, 'ht_kb_category')); ?>" title="<?php echo sprintf( __( 'View all posts in %s', 'helpguru' ), $term->name ); ?>" ><?php echo esc_html($term->name); ?></a>
                
                <?php if(hkb_archive_display_subcategory_count()): ?>
                    <span class="hkb-subcats__cat-count"><?php echo sprintf( _n( '1 Article', '%s Articles', $term->count, 'helpguru' ), $term->count ); ?></span>
                <?php endif; ?>
