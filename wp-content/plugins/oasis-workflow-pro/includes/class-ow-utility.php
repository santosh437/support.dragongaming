...
class OW_Utility {
	...
	public function owf_dropdown_post_status_multi( $selected ) {
		...
		foreach ( $status_array as $status_id => $status_object ) {
			...
			if ( is_array( $selected ) && in_array( $status_id, $selected ) ) { // preselect specified status
				$selected_row .= "\n\t<option selected='selected' value='" . $status_id .
				                 "'>$status_object->label</option>";
			} else {
	...
	public function get_page_link( $total_posts, $current_page_number, $per_page = 20, $action = null, $filters = null
		...
		$page_links_text = sprintf( '<span class="displaying-num">' . __( 'Displaying %s-%s of %s', "oasisworkflow" ) .
		                            '</span>%s',
		...
		if ( count( $role ) > 0 ) {
			...
			foreach ( $role as $k => $v ) {
				if ( $k == 'owfpostauthor' ) { // this is a custom role added by oasis workflow
					...
					$user_role = '%' . $k . '%';
					$query_str = "SELECT users_1.ID, users_1.display_name FROM {$wpdb->users} users_1
   				INNER JOIN {$wpdb->usermeta} usermeta_1 ON ( users_1.ID = usermeta_1.user_id )
   				WHERE (usermeta_1.meta_key like '%capabilities' AND CAST( usermeta_1.meta_value AS CHAR ) LIKE %s)";

					$users = $wpdb->get_results( $wpdb->prepare( $query_str, $user_role ) );
				}
							...
							if ( $user_role == $k || 'owfpostauthor' == $k ||
								...
								if ( $user->ID == $post_author_id ) {
									...
									$part["name"] = $user->display_name . ' (' .
									                __( "Post Submitter", "oasisworkflow" ) . ')';
								} else {
									$part["name"] = $user->display_name;
								}
	...
	public function api_get_priorities( $data ) {
		...
		if ( ! current_user_can( 'ow_submit_to_workflow' ) && ! current_user_can( 'ow_sign_off_step' ) ) {
			return new WP_Error( 'owf_rest_get_priorities',
				__( 'You are not allowed to get workflow priorities', 'oasisworkflow' ), array( 'status' => '403' ) );
		}
	...
	public function get_priority_dropdown( $sel_priority = '2normal' ) {
		...
		$option     = '';
		foreach ( $priorities as $key => $val ) {
			$option .= '<option value="' . $key . '" ' . selected( $sel_priority, $key, false ) . '>' .
			           __( $val, 'oasisworkflow' ) . '</option>';
		}
		echo $option;
	}
	...
	public function get_purge_history_period_dropdown( $selected_period = 'one-month-ago' ) {
		...
		foreach ( $period as $key => $val ) {
			$option .= '<option value="' . $key . '" ' . selected( $selected_period, $key, false ) . '>' .
			           __( $val, 'oasisworkflow' ) . '</option>';
		}
		echo $option;
	}
	...
	public function workflows_dropdown( $workflows, $wf_id = null ) {
		...
		foreach ( $workflows as $workflow ) {
			...
			if ( $workflow->version == 1 ) {
				...
				$option .= "<option value={$workflow_id} {$selected} >" . $workflow_name . " (" . $workflow_version .
				           ")" . "</option>";
			}
		}
		echo $option;
		...
		if ( $ow_expire_date && $ow_expire_date !== "lifetime" ) {
			...
			if ( $current_timestamp > $expiration_timestamp ) { ?>
                <div class="error notice-info">
                    <p>
						<?php
						echo __( 'Your license for ', 'oasisworkflow' ) . $plugin_name .
						     __( ' has been expired. Please update to get access for new features, bug fix, security improvement and our support.',
							     'oasisworkflow' );
						?>
                    </p>
                </div>
