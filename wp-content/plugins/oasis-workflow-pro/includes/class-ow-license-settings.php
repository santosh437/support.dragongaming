		...
		if ( isset( $_POST['oasiswf_license_deactivate'] ) ) { // user is trying to deactivate the license
			...
			if ( $status == OW_License_Service::FAILED ) {
				add_settings_error(
					...
					"Oasis Workflow Pro " .
					__( "License cannot be deactivated. Either the license key is invalid or the licensing server cannot be reached.",
						"oasisworkflow" ),
					'error'
				);
			...
			if ( $status == OW_License_Service::INVALID ) {
				add_settings_error(
					...
					'ow-settings-license-ow-license-key',
					"Oasis Workflow Pro " .
					__( "License cannot be activated. Either the license key is invalid or your activation limit is reached.",
						"oasisworkflow" ),
					'error'
				);
			}
                    ...
                    <div class="select-info full-width">
                        <div class="left quarter-width">
                            <label class="settings-title"
                                   for="oasis_workflow_license_key"><?php echo( "Oasis Workflow Pro " ); ?><?php _e( 'license key',
									"oasisworkflow" ); ?>:</label>
                        </div>
                        <div class="left three-fourth-width">
                            <input type="text" class="regular-text"
                                   name="<?php echo $this->ow_license_key_option_name; ?>"
                                   value="<?php echo $ow_license_key; ?>"/>
							<?php if ( $ow_license_status !== false && $ow_license_status == 'valid' ) { ?>
                                <input type="submit" class="button-secondary" name="oasiswf_license_deactivate"
                                       value="<?php _e( 'Deactivate License', "oasisworkflow" ); ?>"/>
							<?php } ?>
                        </div>
                        <br class="clear">
                    </div>
