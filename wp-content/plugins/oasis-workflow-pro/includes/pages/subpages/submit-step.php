...
global $chkResult;
$action_history_id = ( isset( $_GET["oasiswf"] ) && sanitize_text_field( $_GET["oasiswf"] ) )
	? sanitize_text_field( $_GET["oasiswf"] ) : $chkResult;
$parent_page       = ( isset( $_GET["parent_page"] ) && $_GET["parent_page"] )
	? sanitize_text_field( $_GET["parent_page"] ) : "post_edit"; //check to be called from which page
...
if ( $action_history_id ) {
	...
	$signoff_action_failure   = isset( $step_info->signoff_failure_action ) &&
	                            ( ! empty( $step_info->signoff_failure_action ) ) ? $step_info->signoff_failure_action
		: ( $process == "review" ? __( "Reject", "oasisworkflow" ) : __( "Unable to Complete", "oasisworkflow" ) );
...
<div class="info-setting owf-hidden" id="new-step-submit-div">
                ...
                <div class="select-actors-div">
                    ...
                    <div class="select-actors-div-point">
                        ...
                        <a href="#" id="assignee-unset-point"><img
                                src="<?php echo OASISWF_URL . "img/role-unset.png"; ?>"
                                style="border:0px;"/></a>
                    </div>
                    <div class="select-actors-list">
            ...
            <div class="owf-text-info left full-width <?php echo $hide_publish_section; ?>">
                ...
                <div class="left">
                    ...
                    <label for="immediately-chk">
                        <input type="checkbox"
                               id="immediately-chk" <?php echo $is_immediately ? 'checked="checked"' : ''; ?> />&nbsp;&nbsp;<?php echo __( "Immediately",
							"oasisworkflow" ); ?>
                    </label>
    ...
    <div id="update_publish_msg" class="ow-info-message left owf-hidden">
		<?php echo __( "Signing off will copy over the contents of this revised article to the corresponding published/original article. This will happen either immediately or on the scheduled date/time.",
			"oasisworkflow" ); ?>
    </div>
    ...
    <input type="hidden" name="owf_signoff_ajax_nonce" id="owf_signoff_ajax_nonce"
           value="<?php echo wp_create_nonce( 'owf_signoff_ajax_nonce' ); ?>"/>
</div>
