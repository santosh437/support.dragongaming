...
class OW_History_Service {
	...
	public function delete_history( $period = "" ) {
		...
		$sql                        = "SELECT ID FROM " . OW_Utility::instance()->get_action_history_table_name() .
		...
		$sql = "DELETE from " . OW_Utility::instance()->get_action_table_name() . " WHERE action_history_id in (" .
		...
		$sql = "DELETE from " . OW_Utility::instance()->get_action_history_table_name() . " WHERE id in (" .
		       $place_holders_for_history_ids . ")";
		$wpdb->get_results( $wpdb->prepare( $sql, $history_id_array ) );
	...
	public function create_meta_box() {
		...
		if ( $chkResult && $chkResult != "submit" && $chkResult != "inbox" && $chkResult != "makerevision" ) {
			...
			add_meta_box( $meta_box['id'], $meta_box['title'], array( $this, 'history_graphic_box' ), $meta_box['page'],
	...
	public function get_workflow_history_all( $post_id = null, $page_number, $report_type ) {
		...
		$sql = "SELECT A.* , B.post_title, C.ID as userid, C.display_name as assign_actor, D.step_info, D.workflow_id, D.wf_name, D.version
		       ...
		       OW_Utility::instance()->get_workflow_steps_table_name() . " AS AA LEFT JOIN " .
		       OW_Utility::instance()->get_workflows_table_name() . " AS BB ON AA.workflow_id = BB.ID) AS D
					ON A.step_id = D.ID)
	...
	public function get_review_action_by_history_id( $action_history_id, $order_by = null, $no_action = null ) {
		...
		$results = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " .
		                                               OW_Utility::instance()->get_action_table_name() .
	...
	public function get_action_history_by_status( $action_status, $post_id ) {
		...
		if ( ! empty( $post_id ) ) {
			...
			$post_id       = intval( sanitize_text_field( $post_id ) );
			$results       = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . OW_Utility::instance()
	...
	public function get_action_history_by_post( $post_id ) {
		...
		$results          = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . OW_Utility::instance()
		                                                                                     ->get_action_history_table_name() .
		                                                        " WHERE post_id = %d ORDER BY create_datetime DESC",
			$post_id ) );
		foreach ( $results as $result ) {
			$action_history = $this->get_action_history_from_result_set( $result );
			array_push( $action_histories, $action_history );
	...
	public function get_review_action_by_id( $review_history_id ) {
		...
		$result = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM " . OW_Utility::instance()->get_action_table_name() .
