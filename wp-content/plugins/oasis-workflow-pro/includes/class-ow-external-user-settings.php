	...
	public function validate_external_user_settings( array $input ) {
		...
		if ( ! empty( $user_id_count ) && $user_id_count > 0 ) {
			for ( $index = 0; $index < $user_id_count; $index ++ ) {
				...
				if ( ( $user_fname[ $index ] === "" ) && ( $user_lname[ $index ] === "" ) ) :
					$error_index     = $index + 1;
					$error_message[] = __( "Row ", "oasisworkflow" ) . $error_index .
					                   __( " of external user details requires either First Name or Last Name.",
						                   "oasisworkflow" );
				endif;
				if ( $user_email[ $index ] === "" ) :
					$error_index     = $index + 1;
					$error_message[] = __( "Row ", "oasisworkflow" ) . $error_index .
					                   __( " of external user details requires Email Address.", "oasisworkflow" );
				endif;
            ...
            <div id="external-user-setting">
                <div id="settingstuff">
                  ...
                  <span class="description">
                     <strong><?php echo __( "Manage external users (non-WordPress users) for workflow notifications.",
		                     "oasisworkflow" ); ?></strong>
                     <strong><?php echo __( 'These users will be available on the Email tab under "Other Email Settings" section.',
		                     "oasisworkflow" ); ?></strong>
                  </span>
                    </div>
                    <div class="select-info">
                        ...
                        <div class="external-user-block">
                            <div class="owf-hidden">
                                <input type="hidden" name="<?php echo $this->ow_external_user_option_name ?>[ID][]"
                                       value=""/>
                                <input type="text" class="regular-text eu-name"
                                       name="<?php echo $this->ow_external_user_option_name ?>[fname][]" value=""/>
                                <input type="text" class="regular-text eu-name"
                                       name="<?php echo $this->ow_external_user_option_name ?>[lname][]" value=""/>
                                <input type="text" class="regular-text eu-name"
                                       name="<?php echo $this->ow_external_user_option_name ?>[email][]" value=""/>
                           ...
                           <img src="<?php echo OASISWF_URL ?>/img/trash.png" title="remove"/>
							...
							if ( $external_user_options ) {
								...
								foreach ( $external_user_options as $key => $value ) { ?>
                                    <div class="owf-external-users">
                                        <input type="hidden"
                                               name="<?php echo $this->ow_external_user_option_name ?>[ID][]"
                                               value="<?php echo $key; ?>"/>
                                        <input type="text" class="regular-text eu-name"
                                               name="<?php echo $this->ow_external_user_option_name ?>[fname][]"
                                               value="<?php echo esc_attr( $value['fname'] ); ?>"/>
                                        <input type="text" class="regular-text eu-name"
                                               name="<?php echo $this->ow_external_user_option_name ?>[lname][]"
                                               value="<?php echo esc_attr( $value['lname'] ); ?>"/>
                                        <input type="text" class="regular-text eu-name"
                                               name="<?php echo $this->ow_external_user_option_name ?>[email][]"
                            ...
                            <div class="no-external-user <?php echo $class; ?>">
								<?php echo __( 'No external users found. Click the "+ Add User" button to add external users.',
									'oasisworkflow' ); ?>
                            </div>
                            <div class="owf-eu-button">
                                <input type="button" name="add-external-users" id="add-external-users"
                                       value="<?php _e( '+ Add User', 'oasisworkflow' ); ?>"
