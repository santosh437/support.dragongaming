	...
	private function get_workflow_export_data() {
		...
		if ( ! empty( $valid_workflows ) ) {
			...
			foreach ( $workflows as $workflow ) {
				...
				$results
				             = $wpdb->get_results( $wpdb->prepare( "SELECT *  FROM $steps_table WHERE `workflow_id` = '%d'",
					$workflow_id ) );
				$steps       = array();
				if ( $results ) {
...
//      }
					...
					if ( in_array( $user_role, $available_user_roles ) ) {
						...
						if ( $target_system_user_name !== $user_name ) {
							$validation[] = '<span class="indented-label required-color" >' .
							                __( "Error - ", "oasisworkflow" ) . '"' . $user_name . '"' .
							                __( " not available in the target system.", "oasisworkflow" ) . '</span>';
							continue;
						}
					} else {
						$validation[] = '<span class="indented-label required-color" >' .
						                __( "Error - ", "oasisworkflow" ) . '"' . $user_role . '"' .
						                __( " not available in the target system.", "oasisworkflow" ) . '</span>';
						continue;
					}
	...
	public function get_owf_system_info() {
		...
		foreach ( $plugins as $plugin_path => $plugin ) {
			...
			$update = ( array_key_exists( $plugin_path, $updates ) ) ? ' (needs update - ' .
			                                                           $updates[ $plugin_path ]->update->new_version .
			                                                           ')' : '';
	...
	public function api_check_is_active_editorial_comments( $data ) {
		...
		if ( ! current_user_can( 'ow_submit_to_workflow' ) && ! current_user_can( 'ow_sign_off_step' ) ) {
			...
			return new WP_Error( 'owf_rest_editorial_comments',
				__( 'You are not allowed to view editorial comments.', 'oasisworkflow' ), array( 'status' => '403' ) );
		}
