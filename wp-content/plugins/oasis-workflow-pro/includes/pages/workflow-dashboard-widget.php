...
<div class="dashboard-content">
	...
	if ( $unclaimed_task_count > 0 ) {
		$else_unclaimed_task_message = __( ' But there are currently ', 'oasisworkflow' )
		                               ...
		                               . '</a>';
		$unclaimed_task_message      = __( ' and ', 'oasisworkflow' )
		                               .
		                               sprintf( '<a href="admin.php?page=oasiswf-inbox&action=inbox-unclaimed&user=%s">%s',
			                               $user_id, $unclaimed_task_count )
		                               . __( ' unclaimed task(s).', 'oasisworkflow' )
    ...
    <div class="main-content">
		<?php
		if ( $task_count > 0 ) {
			$task_count_message = __( 'There are currently ', 'oasisworkflow' )
			                      . sprintf( '<a href="admin.php?page=oasiswf-inbox">%s', $task_count )
			                      . __( ' assignment(s)', 'oasisworkflow' )
			                      . '</a>'
			                      . $unclaimed_task_message;
			echo "<span>";
		...
		if ( ( get_option( 'oasiswf_default_due_days' ) != '' ) || ( $reminder_email_settings == 'yes' ) ) : ?>
            <div class="main-content1">
                ...
                <ul>
					...
					$due_types = array(
						'overdue'           => array(
							'label' => __( 'Overdue', 'oasisworkflow' ),
							'count' => 0,
							'color' => ''
						),
						'due_today'         => array(
							'label' => __( 'Due Today', 'oasisworkflow' ),
							'count' => 0,
							'color' => ''
						),
						'due_tomorrow'      => array(
							'label' => __( 'Due Tomorrow', 'oasisworkflow' ),
							'count' => 0,
							'color' => ''
						),
						'due_in_seven_days' => array(
							'label' => __( 'Due in 7 days', 'oasisworkflow' ),
							'count' => 0,
							'color' => ''
						)
					);
					...
					foreach ( $due_types as $key => $value ) {
						...
						echo "<li class='" . $css_class . "' style='color:" . $value['color'] . "'>" . $value['count'] .
						     " " . $value['label'] . "</li>";
					...
					foreach ( $priority_array as $priority_key => $priority_name ) {
						...
						foreach ( $task_list_by_priority as $priority_task ) {
							if ( $priority_key ===
								...
								echo "<li class=" . $css_class . "-task-priority>" . $priority_task->priority_count .
								     " " . $priority_name . "</li>";
								$has_tasks = true;
								break;
							}
