        ...
        */
        function check_exits_table_exists(){
            ...
            if(! $exits_table_exists && ! get_transient('_ht_kb_just_installed') ){
                return new WP_Error( 'ht-kb-db-exits-not-found', sprintf( __( 'HelpGuru Knowledge Base Exits table does not exist, please <a href="%s">Deactivate</a> then re-Activate the HelpGuru Knowledge Base plugin', 'ht-knowledge-base'), admin_url('plugins.php#helpguru-knowledge-base') ) );
            }
            return false;
        }
