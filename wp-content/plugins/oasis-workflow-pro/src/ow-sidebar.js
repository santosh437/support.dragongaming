...
export class OasisWorkflowProComponent extends Component {
    ...
    componentDidMount() {
        ...
        wp.apiFetch({
            ...
            (response) => {
                if (response.can_skip_workflow && !this.props.postMeta._oasis_original) {
                    // the post is a revision post, then hide Publish
                    // show the Publish button when loading the Workflow section
                    [].forEach.bind(document.getElementsByClassName("editor-post-publish-panel__toggle"), function (
                        itm
                    ) {
